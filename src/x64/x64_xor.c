...
{
    if ((compare(type, u32_handle) || compare(type, i32_handle)) && destination == source)
    {
        // XOR r32, r32.
        write_text_u8(0x31);
...
{
    Ir_Operand* source = lookup_in(&xor->sources, 0);
    Ir_Operand* destination = lookup(xor->destination);
...
uint16_t size_of_x64_xor(const Ir_Instruction* xor)
{
    ...
    if (IS_X64_REG(destination->tag) && IS_X64_REG(source->tag))
    {
        if ((compare(xor->type, u32_handle) || compare(xor->type, i32_handle)) && destination->tag == source->tag)
            // XOR r32, r32.
            return 2;
        else
            not_implemented(__FILE__, __LINE__, "Differing xor registers");
