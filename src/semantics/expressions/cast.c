...
static bool is_castable(const Type* a, const Type* b)
{
    if (a->tag == Type_Tag_enumeration)
        return is_castable(lookup_type(a->enumeration.underlying_type), b);
    else if (b->tag == Type_Tag_enumeration)
        return is_castable(a, lookup_type(b->enumeration.underlying_type));
...
void type_check_cast(const File* file, Cast* cast, Expression_Check_Flags flags)
{
    ...
    Type* target_type = lookup_type(cast->type);

    Expression* source = lookup_expression(cast->source);
    type_check_expression(file, source, cast->type, flags);
    if (target_type->tag != Type_Tag_function && !is_castable(lookup_type(source->type), target_type))
...
void deep_copy_cast(Cast* restrict destination, const Cast* restrict source)
{
    ...
    deep_copy_expression(lookup_expression(destination->source), lookup_expression(source->source));
