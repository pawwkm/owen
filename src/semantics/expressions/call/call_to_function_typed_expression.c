...
void call_to_function_typed_expression(const File* file, Call* call, Expression_Check_Flags flags)
{
    ...
    Function_Type* signature = &lookup(type_check_expression(file, callee, invalid_type_handle, flags))->function;
    ...
    {
        Type_Handle expected_type = handle_at(&signature->formal_parameters, i);
        Expression* actual_parameter = lookup_in(&call->actual_parameters, i);
